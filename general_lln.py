...
path = Path(__file__).parents[0]/'videos/lln/'
...
for key, item in my_dist_args.items():
    ...
    fig, ax = plt.subplots(
        1,1, figsize=(4,3.5), gridspec_kw=dict(left=0.20, right=0.98, bottom=0.24))
    ...
    fps = 12
    frames = (n_in_each_frame.shape[0]//fps)*fps
    anim = FuncAnimation(fig, uh, frames=frames, blit=True)
    anim.save(path/f"evolving_{key:s}.mp4", fps=12, dpi=300, codec='libx264', bitrate=-1, extra_args=['-pix_fmt', 'yuv420p'])
# %%
