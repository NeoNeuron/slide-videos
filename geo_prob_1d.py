...
class UpdateFigure_geo_prob:
    def __init__(self, data:np.ndarray, ax:plt.Axes):
        ...
        self.hit = 0
        ax.set_title(f"{self.hit:3d} / {0:3d}", fontsize=20)
        self.ax = ax
    ...
    def __call__(self, i):
        ...
        if i > 0 and i<=self.data.shape[0]:
            ...
            if self.data[i-1] < 0.5:
                self.current_sample = self.ax.axvline(self.data[i-1], color=GREEN)
                self.hit += 1
            else:
                self.current_sample = self.ax.axvline(self.data[i-1], color=RED)

            self.ax.set_title(f"{self.hit:3d} / {i:3d}", fontsize=20)
            # update last sample
...
#%%
fig, ax = plt.subplots(1,1, figsize=(10,2), dpi=100, gridspec_kw={'top':0.8, 'bottom':0.45})
ax.plot([-0.5,],[0,], ms=100, color='k', marker=subway_marker)[0].set_clip_on(False)
...
ax.set_xlabel('地铁到站后时间(分钟)', fontsize=25)
...
anim.save('geo_prob_1d.mp4', fps=48, dpi=200, codec='libx264', bitrate=-1, extra_args=['-pix_fmt', 'yuv420p'])
# %%
