...
indices = np.random.choice(np.arange(10000), 200, replace=False)
...
for i in range(mask.shape[0]):
    test_res[i] = rvar[i] >= 0.95 if mask[i] else rvar[i] >= 0.999
...
person_marker = gen_marker('icons/mail.svg')
patient_marker = gen_marker('icons/junk.svg')
...
class UpdateDist:
    def __init__(self, ax0, ax, ax1, patient_mask):
        ...
        self.ax.set_yticklabels(["检测阳性（患病）","检测阳性（健康）", "检测阴性"], fontsize=20)
        self.ax.set_xlabel("被检测人数", fontsize=20)
        ...
        self.line, =ax1.plot([],[], lw=5, color='r')
        self.ax1 = ax1
        self.ax1.set_xlim([0,10000])
        self.ax1.set_ylim([0.0,1.0])
        self.ax1.set_xlabel('被检测人数', fontsize=20)
        self.ax1.set_ylabel('检测准确率', fontsize=20)
        self.ax1.spines['top'].set_visible(False)
        self.ax1.spines['right'].set_visible(False)
        self.ax1.axhline(0.505, linestyle='--', color='black')
        # self.ax1.axhline(0.98, linestyle='--', color='black')
    ...
    def __call__(self, i):
        ...
        if i == 0:
            self.success = 0
            for rect, h in zip(self.rects, [0, 0, 0]): 
                rect.set_width(h)
...
anim.save('mail_movie.mp4', fps=10, dpi=200, codec='libx264', bitrate=-1, extra_args=['-pix_fmt', 'yuv420p'])
# %%