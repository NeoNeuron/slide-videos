...
stock_y = np.array([30.47, 33.30, 32.41, 39.39, 30.40, 35.25, 36.65, 27.53, 19.62, 30.73])
stock_z = np.array([83.18 ,80.14 ,78.51 ,76.13 ,75.04 ,78.92 ,78.08 ,83.46 ,87.85 ,79.77])
stock_w = np.array([28.17, 24.24, 32.45, 31.08, 26.42, 30.28, 28.74, 25.70, 25.00, 30.99])
#%%
...
ax[0].set_ylabel('$X$股价')
ax[0].set_ylim(60,100)
ax[1].plot(np.arange(1,11), stock_z, '-o',ms=15, markeredgewidth=2.5, markerfacecolor='#353A71', markeredgecolor='#A7BED2')
ax[1].set_xlabel('时间')
ax[1].set_ylabel('$Z$股价')
ax[1].set_ylim(70,90)
plt.savefig(path/'stock_xz_sep.pdf')
fig, ax = plt.subplots(1,1, figsize=(6.5,6),
                       gridspec_kw=dict(left=0.2, right=0.95, bottom=0.15, top=0.95))
ax.plot(stock_x, stock_z, 'o',ms=15, markeredgewidth=2.5, markerfacecolor='#C00000', markeredgecolor='#D77F66')
ax.set_xlabel('$X$股价')
ax.set_xlim(60,100)
ax.set_ylabel('$Z$股价')
ax.set_ylim(70,90)
ax.set_yticks(np.linspace(70,90,5))
pval = np.polyfit(stock_x, stock_z, deg=1)
ax.plot(np.sort(stock_x), np.polyval(pval, np.sort(stock_x)), '--', c='#C00000')
plt.savefig(path/'stock_xz_cov.pdf')
# %%
